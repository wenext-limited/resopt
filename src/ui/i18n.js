...
const MESSAGES = {
  en: {
    ...
    fileStatusUnavailable: 'Live file checks are unavailable. Keeping the last known file list and retrying automatically.',
    ...
    modeCandidates: 'Opportunities', modeWarnings: 'Warnings', modeDuplicates: 'Similar groups', modeApplied: 'Applied', modeImages: 'Images', modeUnsupported: 'No optimizer', modeFailed: 'Failed', modeAll: 'All',
    ...
    archiveCandidate: 'Verified lossless ZIP: {0} → {1} · saves {2}', archiveDownload: 'Download optimized ZIP', archiveOptimized: '{0} embedded PNGs recompressed losslessly. Filenames, pixels, and other payloads are preserved.', archiveImageUnchanged: 'Image kept unchanged: {0}',
    archivePreviewUnavailable: 'Preview unavailable: {0}', archiveContents: 'Archive contents', archiveSummary: '{0} entries · ZIP {1} · expanded {2}', archivePreviewLimit: 'Previews cover up to {0} of the largest images. Entries are never extracted into your project.', archiveSearch: 'Search archive paths…', archiveType: 'Archive format', archiveProtected: 'Package integrity metadata found. Inspect only; rewriting requires the package publisher.', archiveMetadata: 'Finder / AppleDouble metadata', archiveStored: '{0} stored in ZIP',
    dup_identical: 'Identical files', dup_resized: 'Similar pictures at different sizes', dup_similar: 'Similar pictures · review differences', dupRedundant: '{0} files · {1} in other members (not confirmed savings)',
    dupGroups: 'Groups', dupTotalSize: 'Total size', dupImageCount: '{0} images', dupGroupRange: 'Groups {0}–{1} of {2}', dupReferenceName: 'Reference: {0}', dupRangeHint: 'Lowest–highest member similarity to the reference', dupSortGroup: 'Review size ↓',
    dupSimilarity: 'Similarity', dupReference: 'Reference · largest file', dupGroup: 'Group {0}', dupScore: '{0} / 100 similarity', dupExactScore: '100 / 100 · identical bytes', dupUnscored: 'Score unavailable · re-analyze', dupNoPreview: 'Preview unavailable', dupOpenPreview: 'Open preview',
    dupScoreHint: 'Scores compare each image to the reference, not to the selected image. Higher is closer: 100 means identical file bytes; other scores are approximate, capped at 99.9.',
    dupDifferences: 'Differences — brightness {0} · opacity {1} · mean color {2} · largest cell {3}',
    dupHint: 'Score = 100 × (1 − the largest average brightness, opacity or mean-color difference). Uses 16×16 image summaries; tiny details and local color changes can be missed. This is not the compression quality score or proof that files can replace one another. Scale variants alone are excluded; removing files still requires reviewing usage.',
    effectReadOnly: 'Preview only',
    pagPlayer: 'PAG / TCMP4 animation', pagDetected: 'PAG content detected in a {0} file.', pagLicense: 'PAG runtime licenses', pagContents: '{0} editable texts · {1} editable images · {2} videos', pagUnavailable: 'The bundled PAG renderer cannot preview this file. Use resopt serve and check browser/SDK compatibility; no conversion was performed.', pagPreviewNote: 'Original template content only; app replacements may differ. Uses bundled libpag 4.3.51 with no CDN requests. Preview is limited to 640 pixels.',
    vapPlayer: 'VAP transparent animation', effectTimeline: 'Animation frame', effectLoading: 'Loading animation…', effectUnavailable: 'This animation cannot be previewed in this browser, or its metadata does not match the video.', effectPreviewNote: 'Preview scaled to at most 640 pixels. Original animation bytes are unchanged.', effectDynamic: '{0} dynamic sources: app-provided text/images are not rendered in this preview.',
  ...
  'zh-CN': {
    ...
    fileStatusUnavailable: '暂时无法检查文件状态，保留上次的文件列表并自动重试。',
    ...
    modeCandidates: '可优化', modeWarnings: '有警告', modeDuplicates: '相似组', modeApplied: '已应用', modeImages: '图片', modeUnsupported: '暂无优化器', modeFailed: '失败', modeAll: '全部',
    ...
    archiveCandidate: '已验证无损 ZIP：{0} → {1} · 节省 {2}', archiveDownload: '下载优化后的 ZIP', archiveOptimized: '{0} 张内嵌 PNG 已无损重压缩，文件名、像素及其他文件内容均保留。', archiveImageUnchanged: '图片保持原样：{0}',
    archivePreviewUnavailable: '无法生成预览：{0}', archiveContents: '压缩包内容', archiveSummary: '{0} 个条目 · ZIP {1} · 解压后 {2}', archivePreviewLimit: '最多预览体积最大的 {0} 张图片。不会将内容解压到项目中。', archiveSearch: '搜索压缩包内路径…', archiveType: '压缩包内格式', archiveProtected: '检测到包完整性元数据，仅供检查；重建需由资源发布流程处理。', archiveMetadata: 'Finder / AppleDouble 元数据', archiveStored: 'ZIP 内占用 {0}',
    dup_identical: '完全相同的文件', dup_resized: '不同尺寸的相似图片', dup_similar: '相似图片 · 请检查差异', dupRedundant: '{0} 个文件 · 其他成员共 {1}（不代表可节省体积）',
    dupGroups: '图片组', dupTotalSize: '总大小', dupImageCount: '{0} 张图片', dupGroupRange: '第 {0}–{1} 组 / 共 {2} 组', dupReferenceName: '参考图：{0}', dupRangeHint: '组内成员与参考图相似度的最低值至最高值', dupSortGroup: '组内待审核体积 ↓',
    dupSimilarity: '相似度', dupReference: '参考图 · 最大文件', dupGroup: '第 {0} 组', dupScore: '{0} / 100 相似度', dupExactScore: '100 / 100 · 文件字节相同', dupUnscored: '暂无评分 · 请重新分析', dupNoPreview: '暂无预览', dupOpenPreview: '打开预览',
    dupScoreHint: '每张图片均与参考图比较，而非当前选中的图片。分数越高越相似：100 表示文件字节完全相同；其余为估算，最高 99.9。',
    dupDifferences: '差异 — 亮度 {0} · 透明度 {1} · 平均颜色 {2} · 最大网格差异 {3}',
    dupHint: '评分 = 100 ×（1 − 平均亮度、透明度、平均颜色差异中的最大值）。基于 16×16 图片摘要，可能忽略微小细节和局部颜色变化。这不是压缩质量评分，也不能证明文件可互相替换。同一资源的倍率变体不会单独成组；删除文件仍需检查引用。',
    effectReadOnly: '仅预览',
    pagPlayer: 'PAG / TCMP4 动画', pagDetected: '在 {0} 文件中检测到 PAG 内容。', pagLicense: 'PAG 运行库许可', pagContents: '{0} 处可替换文字 · {1} 处可替换图片 · {2} 段视频', pagUnavailable: '内置 PAG 渲染器无法预览此文件。请使用 resopt serve 并检查浏览器/SDK 兼容性；未进行格式转换。', pagPreviewNote: '仅预览原始模板，App 替换内容可能不同。使用内置 libpag 4.3.51，不请求 CDN；预览最长边为 640 像素。',
    vapPlayer: 'VAP 透明动效', effectTimeline: '动画帧', effectLoading: '加载动画中…', effectUnavailable: '浏览器无法预览此动画，或动画元数据与视频不符。', effectPreviewNote: '预览最长边不超过 640 像素，原始动画字节未改动。', effectDynamic: '{0} 个动态资源：预览不包含 App 注入的文字或图片。',
